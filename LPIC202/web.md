...
　   全ての条件に合致したら真  
  ...
  -  Require None  
     全ての条件に合致しなければ真
  -  user  
     ユーザリストを設定して許可拒否を設定する。valid-userで認証したとしても、userで許可されないユーザであればアクセス拒否する
  -  valid-user  
     パスワードファイル（.htpasswd）内にある全てのユーザが認証対象。 認証されたユーザのみ許可。（言い換えれば認証さえ通ればアクセスできる）
...
-  LimitExcept  
指定したhttpメソッド以外のメソッドにアクセス制御が可能になる。  
全拒否することも可能。  
...
-  UserDir  
ユーザの公開ディレクトリを指定。  
http://ホスト名/-ユーザ名/index.htmlの場合、ユーザの下のUserDirで指定した公開ディレクトリの下のindex.htmlを指定する。  
...
-  htpasswd/htdigestオプション  
-c　パスワードファイルを新規作成、ファイルがすでに存在する場合は上書き  追加や変更の場合はオプション不要  
-  .htpasswd  
...
httpサーバとしての設定ができる。  
-  /etc/nginx/conf.d  
このディレクトリに配置された設定ファイルも設定として読み込む。  
nginx.confで無効化できる。  
...
-  auth_param  認証方式（basicなど）　program　認証プログラム  
認証方法などを設定する。
-  cache_log  
キャッシュのログを格納するファイルのパスを指定する。
