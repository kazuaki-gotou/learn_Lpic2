...
ゾーンの情報を管理するサーバ名を記載。  
- MX  
ゾーンで管理するドメイン宛のメールの宛先メールサーバを記述。優先度を指定し、低いものから送信。送信失敗の場合はその次の宛先に送信。 
-  A  
正引き。ドメインからIPアドレスを調べるためのレコード。　　
-  グルーレコード  
委任先のネームサーバーのIPアドレスを解決するためのレコード。名前解決ループとならないようにする。
-  TLSAレコード  
DANEより取得できる。デジタル署名されたレコードのこと。  
    -  DANE DNSSECにてDNSサーバからの応答が正しいかどうかを認証できるようになることで、DNSを認証の手段として利用できるようにした。
##  bind  
linuxのdnsソフトウェア  
###  named  
...
スレーブサーバのゾーンファイルをテキスト形式にする。  
スレーブサーバのゾーンファイルはバイナリ形式になっている。       
→スレーブサーバのゾーンファイルは通常、マスターサーバから複製される。  


-  named-checkzone（コマンド）  
ゾーンファイルの内容を参照できる。ただし、バイナリ形式のファイルはバイナリ形式で出力される。  
-  named-compilezone（コマンド）  
バイナリ形式のゾーンファイルをテキスト形式にできる。ただし、元々のファイルがバイナリの場合、ゾーン転送でファイル更新されると再びバイナリになる。  
