...
-  anon_upload_enable=yesno  
匿名ユーザによるアップロードの許可拒否  
-  anonymous_enable=yesno  
匿名ユーザのログインを許可するかどうか  

####  pure-ftpd  
よく使われるFTPサーバ  
-  pure-ftpdコマンド  
     - -E　匿名によるログインの禁止 -e 匿名ユーザのみログイン可能 -i 匿名ユーザのファイルアップロードの禁止
-  pure-ftpd.conf  
設定ファイル  
...
-  ssh -L  
ローカルポートフォワーディング  
例）自分のpc:8080はsshトンネルを経て相手のサーバ:80に接続。
-  ssh -R  
リモートポートフォワーディング  
例）sshサーバの:9090はsshトンネルを経て自分pcの:3008に接続。  
...
以下はディレクティブ  
    - client-to-client 
    クライアント同士の通信を許可する。標準では許可されていない。
    - push  
    クライアント側にルート伝達もできる。  
    push "route 192.168.3.0 255.255.255.0"  
    - status  
    接続中クライアント、ルーティング情報が1分間隔で情報更新され記録される。  
